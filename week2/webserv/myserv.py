...
def runScript(sScript):
	proc = Popen('python ' + sScript, stdout=PIPE, universal_newlines=True)
	output = proc.communicate()[0]
	return PYTHON_OUTPUT % (sScript, PYTHON_LINE % (output))
	     
def get_content(uri):
	print 'fetching:', uri
	try:
		path = '.' + uri
		if uri.endswith('/gettime'):
			print 'Returning the time'
			return (200, 'text/html', time.strftime('%X %Z'))
		if path.endswith('.py'):
			print path
			return (200, 'text/html', runScript(path))
